...
class AmqpConsumeCorrelatedAction(req: ConsumeRequest,
                                 ...
                                 )(implicit amqp: AmqpProtocol) extends ChainableAction with NameGen {
  ...
  override def !(session: Session): Unit = this.execute(session)
  ...
  override def execute(session: Session): Unit = {
    ...
    req match {
      ...
      case _ =>
        ...
        logger.warn("There is something wrong. In single step of scenario there seems to be two different consume call types " +
